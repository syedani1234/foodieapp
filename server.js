...
if (process.env.DATABASE_URL) {
  try {
    ...
    console.error(
      "❌ Failed to create database pool from DATABASE_URL:",
      err.message,
    );
    pool = null;
  }
  ...
  try {
    ...
    const addColumnIfMissing = async (table, column, definition) => {
      try {
        ...
        console.log(
          `⚠️  Could not check/add column ${column} in ${table}:`,
          err.message,
        );
      }
    };
    ...
    await addColumnIfMissing(
      "orders",
      "status",
      "ENUM('pending','preparing','ready','delivered','cancelled') DEFAULT 'pending'",
    );
    ...
    try {
      const [menuItemCol] = await pool.query(
        "SHOW COLUMNS FROM order_items WHERE Field = 'menu_item_id' AND `Null` = 'NO'",
      );
    ...
    await addColumnIfMissing(
      "order_items",
      "deal_id",
      "INT NULL AFTER menu_item_id",
    );

    try {
      await pool.query(
        "ALTER TABLE order_items ADD FOREIGN KEY (deal_id) REFERENCES deals(id) ON DELETE SET NULL",
      );
    } catch (err) {
      console.log(
        "⚠️  Could not add deal_id foreign key (may already exist):",
        err.message,
      );
    }
...
app.get("/api/cuisines", async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ error: "Failed to fetch cuisines", message: error.message });
  }
});
...
app.post("/api/cuisines", async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ error: "Failed to create cuisine", message: error.message });
  }
});
  ...
  try {
    ...
    if (cuisineRows.length === 0) {
      [cuisineRows] = await pool.query(
        `SELECT id, name, description, image, is_featured FROM cuisines WHERE slug = ? AND is_active = TRUE`,
        [slug],
      );
    }
    if (cuisineRows.length === 0) {
...
app.get("/restaurants", async (req, res) => {
  try {
    ...
    const restaurants = rows.map((r) => ({
      ...
      cuisine_ids: r.cuisine_ids
        ? r.cuisine_ids.split(",").map((id) => parseInt(id))
        : [],
    }));
    ...
    res
      .status(500)
      .json({ error: "Failed to fetch restaurants", message: error.message });
  }
});
...
app.post("/api/restaurants", async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ error: "Failed to create restaurant", message: error.message });
  }
});
...
app.post("/api/menu-items", upload.single("image"), async (req, res) => {
  try {
    const { restaurant_id, name, description, base_price, is_available } =
      req.body;
    ...
    res
      .status(500)
      .json({ error: "Failed to create menu item", message: error.message });
  }
});
...
app.get("/deals", async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ error: "Failed to fetch deals", message: error.message });
  }
});
...
app.get("/api/deals/:id", async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ error: "Failed to fetch deal", message: error.message });
  }
});
...
app.post("/api/deals", upload.single("image"), async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ error: "Failed to create deal", message: error.message });
  }
});
...
app.patch("/api/deals/:id", upload.single("image"), async (req, res) => {
  ...
  try {
    ...
    res
      .status(500)
      .json({ error: "Failed to update deal", message: error.message });
  }
});
...
app.delete("/api/deals/:id", async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ error: "Failed to delete deal", message: error.message });
  }
});
...
app.post("/api/upload", upload.single("image"), (req, res) => {
  try {
    ...
    if (error.code === "LIMIT_FILE_SIZE")
      return res
        .status(400)
        .json({ error: "File too large", message: "Maximum file size is 5MB" });
    res
      .status(500)
...
app.post("/api/orders", async (req, res) => {
  ...
  try {
    ...
    if (restaurantCheck.length === 0)
      return res
        .status(400)
        .json({ success: false, error: "Invalid or inactive restaurant" });
...
app.get("/api/debug/db", async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ error: "Failed to fetch debug data", message: error.message });
  }
});
...
app.use((error, req, res, next) => {
  ...
  if (error instanceof multer.MulterError) {
    ...
    return res
      .status(400)
      .json({ error: "File upload error", message: error.message });
  }
  res
    .status(500)
    .json({
