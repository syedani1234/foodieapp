...
console.log("4. Middleware set up, creating uploads dir...");
console.log("5. Uploads dir ready, creating pool...");
...
// Use DATABASE_URL if provided, otherwise fallback to individual variables
if (process.env.DATABASE_URL) {
  try {
    pool = mysql.createPool(process.env.DATABASE_URL);
    console.log("6. Database pool created from DATABASE_URL");
  } catch (err) {
    console.error("❌ Failed to create database pool from DATABASE_URL:", err.message);
    pool = null;
  }
} else {
  try {
    ...
    console.log("6. Database pool created from individual vars");
  } catch (err) {
...
async function checkAndAddMissingColumns() {
  ...
  try {
    ...
    } catch (err) { /* indexes may already exist */ }
...
initializeDatabase().catch(err => console.error("❌ Unhandled error in initializeDatabase:", err));
...
app.post("/api/cuisines", async (req, res) => {
  try {
    ...
    if (!name || name.trim() === '') {
      return res.status(400).json({ error: "Validation error", message: "Cuisine name is required" });
    }
    ...
    if (existing.length > 0) {
      return res.status(409).json({ error: "Cuisine already exists", message: `A cuisine with name "${cuisineName}" already exists` });
    }
    ...
    res.status(201).json({
      ...
      data: { id: result.insertId, name: cuisineName, slug: nameToSlug(cuisineName) }
    });
  } catch (error) {
...
app.get("/cuisines/:slug", async (req, res) => {
  ...
  try {
    ...
    if (cuisineRows.length === 0) {
      [cuisineRows] = await pool.query(`SELECT id, name, description, image, is_featured FROM cuisines WHERE slug = ? AND is_active = TRUE`, [slug]);
    }
    if (cuisineRows.length === 0) {
      const likeName = slug.replace(/-/g, ' ');
      [cuisineRows] = await pool.query(
        `SELECT id, name, description, image, is_featured FROM cuisines WHERE LOWER(TRIM(name)) LIKE LOWER(?) AND is_active = TRUE`,
        [`%${likeName}%`]
      );
    }
    if (cuisineRows.length === 0) {
      return res.status(404).json({ error: "Cuisine not found", message: `Cuisine "${slug}" does not exist in our database.` });
    }
...
app.get("/restaurants", async (req, res) => {
  try {
    ...
    const [rows] = await pool.query(
      `SELECT DISTINCT
        r.id, r.name, r.image, r.address, r.description, r.rating, r.delivery_time, r.delivery_fee,
...
app.get("/api/restaurants/:id", async (req, res) => {
  try {
    ...
    if (isNaN(restaurantId) || restaurantId <= 0) {
      return res.status(400).json({ error: "Invalid restaurant ID", message: "Restaurant ID must be a positive number" });
    }
    const [restaurantRows] = await pool.query(
      `SELECT r.*, 
        GROUP_CONCAT(DISTINCT c.name SEPARATOR ', ') as cuisine_names,
    ...
    if (restaurantRows.length === 0) {
      return res.status(404).json({ error: "Restaurant not found", message: `Restaurant with ID ${restaurantId} does not exist or is inactive` });
    }
...
app.post("/api/restaurants", async (req, res) => {
  try {
    ...
    if (missingFields.length > 0) {
      return res.status(400).json({ error: "Validation error", message: `Missing required fields: ${missingFields.join(', ')}` });
    }
    const {
    ...
    const [result] = await pool.query(
      `INSERT INTO restaurants 
       ...
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
      [
        name.trim(), description || '', address || '', city || '', phone || '', email || '',
        website || '', opening_hours || '', image || null, cover_image || null,
        parseFloat(rating) || 0, delivery_time || '30-45 minutes', parseFloat(minimum_order) || 0,
        parseFloat(delivery_fee) || 2.99, is_featured || false
      ]
    );
...
app.post("/api/menu-items", upload.single('image'), async (req, res) => {
  try {
    ...
    if (!restaurant_id || !name || !base_price) {
      return res.status(400).json({ error: "Missing required fields", message: "restaurant_id, name, and base_price are required" });
    }
    const [restaurant] = await pool.query("SELECT id FROM restaurants WHERE id = ? AND is_active = TRUE", [parseInt(restaurant_id)]);
    if (restaurant.length === 0) {
      return res.status(400).json({ error: "Invalid restaurant", message: `Restaurant with ID ${restaurant_id} does not exist or is inactive` });
    }
    const image = req.file ? `/uploads/${req.file.filename}` : null;
    const [result] = await pool.query(
      `INSERT INTO menu_items (restaurant_id, name, description, base_price, image, is_available)
       VALUES (?, ?, ?, ?, ?, ?)`,
      [parseInt(restaurant_id), name.trim(), description || '', parseFloat(base_price), image, is_available === 'true' || is_available === true || is_available === '1']
    );
    ...
    res.status(201).json({
      ...
      data: {
        id: item.id, restaurant_id: item.restaurant_id, name: item.name, description: item.description,
        base_price: parseFloat(item.base_price), image: formatImageUrl(item.image),
        is_available: Boolean(item.is_available), created_at: item.created_at, updated_at: item.updated_at
      }
    });
  } catch (error) {
...
// ========== DEAL ENDPOINTS (simplified – keep your existing ones) ==========
app.get("/deals", async (req, res) => {
  try {
    ...
    const formattedDeals = deals.map(deal => ({
      ...
      restaurant_id: deal.restaurant_id || null, restaurant_name: deal.restaurant_name || '',
      restaurant_image: formatImageUrl(deal.restaurant_image), cuisine_id: deal.cuisine_id || null,
      cuisine_name: deal.cuisine_name || null, original_price: parseFloat(deal.original_price) || 0,
      discount_price: parseFloat(deal.discount_price) || 0, discount_percent: parseInt(deal.discount_percent) || 0,
      image: formatImageUrl(deal.image), is_active: Boolean(deal.is_active), is_featured: Boolean(deal.is_featured),
      tags: deal.tags || '', valid_from: deal.valid_from, valid_until: deal.valid_until,
      quantity_available: deal.quantity_available, has_customization: Boolean(deal.has_customization || false),
      deal_type: deal.deal_type || 'other', created_at: deal.created_at, updated_at: deal.updated_at
    }));
...
app.get("/api/deals", async (req, res) => {
  try {
    const { featured, restaurant_id, cuisine_id, limit = 20, page = 1, deal_type, has_customization } = req.query;
    ...
    } catch (error) { cuisineIdExists = false; }
    ...
    } catch (error) { dealTypeExists = false; }
    ...
    } catch (error) { hasCustomizationExists = false; }
    ...
    const formattedDeals = deals.map(deal => {
      ...
      return {
        ...
        restaurant_image: formatImageUrl(deal.restaurant_image), cuisine_id: deal.cuisine_id || null,
        cuisine_name: deal.cuisine_name || null, original_price: parseFloat(deal.original_price) || 0,
        discount_price: parseFloat(deal.discount_price) || 0, discount_percent: discountPercent || 0,
        image: formatImageUrl(deal.image), is_active: Boolean(deal.is_active), is_featured: Boolean(deal.is_featured),
        tags: deal.tags || '', valid_from: deal.valid_from, valid_until: deal.valid_until,
        quantity_available: deal.quantity_available, has_customization: hasCustomization,
        deal_type: detectedDealType, created_at: deal.created_at, updated_at: deal.updated_at
      };
    });
...
app.get("/api/deals/:id", async (req, res) => {
  try {
    ...
    if (isNaN(dealId) || dealId <= 0) return res.status(400).json({ error: "Invalid deal ID" });
    let query;
    try {
      const [columns] = await pool.query("SHOW COLUMNS FROM deals LIKE 'cuisine_id'");
      if (columns.length > 0) {
        query = `SELECT d.*, r.name AS restaurant_name, r.image AS restaurant_image, c.name as cuisine_name
                 FROM deals d LEFT JOIN restaurants r ON d.restaurant_id = r.id LEFT JOIN cuisines c ON d.cuisine_id = c.id
                 WHERE d.id = ? AND d.is_active = TRUE`;
      } else {
        query = `SELECT d.*, r.name AS restaurant_name, r.image AS restaurant_image
...
app.get("/api/deals/:id/customization", async (req, res) => {
  // (Keep your existing deal customization logic – unchanged)
  try {
    const dealId = parseInt(req.params.id);
    if (isNaN(dealId) || dealId <= 0) return res.status(400).json({ error: "Invalid deal ID" });
    ...
    if (dealType === 'pizza' || (hasPizza && !hasBurger)) {
      ...
      items = [
        {
          id: 1, name: 'Cheese Pizza', description: 'Delicious cheese pizza with mozzarella and tomato sauce', base_price: pizzaPrice,
          crusts: [{ id: 1, name: "Regular", price: 0 }, { id: 2, name: "Thin Crust", price: 20 }, { id: 3, name: "Cheesy Crust", price: 30 }],
          toppings: [{ id: 1, name: "Extra Cheese", price: 20 }, { id: 2, name: "Mushrooms", price: 15 }, { id: 3, name: "Olives", price: 10 }, { id: 4, name: "Pepperoni", price: 25 }]
        },
        {
          id: 2, name: 'Beef Burger', description: 'Juicy beef burger with lettuce, tomato, and special sauce', base_price: burgerPrice,
          sizes: [{ id: 1, name: "Regular", price: 0 }, { id: 2, name: "Large", price: 50 }, { id: 3, name: "Jumbo", price: 80 }],
          addons: [{ id: 1, name: "Extra Patty", price: 50 }, { id: 2, name: "Bacon", price: 30 }, { id: 3, name: "Cheese Slice", price: 20 }, { id: 4, name: "Avocado", price: 25 }, { id: 5, name: "Fried Egg", price: 15 }]
        }
      ];
    ...
    res.json({
      ...
      deal: {
        ...
        discount_price: parseFloat(deal.discount_price) || 0, original_price: parseFloat(deal.original_price) || 0,
        restaurant_name: deal.restaurant_name || 'Restaurant', restaurant_image: formatImageUrl(deal.restaurant_image),
...
app.post("/api/deals", upload.single('image'), async (req, res) => {
  try {
    const {
      title, description, original_price, discount_price, discount_percent, restaurant_id,
      cuisine_id, valid_from, valid_until, tags, deal_type, is_featured, is_active = true,
      has_customization, quantity_available
    } = req.body;
    const missingFields = validateRequiredFields(['title', 'original_price', 'restaurant_id'], req.body);
    if (missingFields.length) return res.status(400).json({ error: "Validation error", message: `Missing: ${missingFields.join(', ')}` });
    const [restaurantRows] = await pool.query("SELECT id FROM restaurants WHERE id = ? AND is_active = TRUE", [parseInt(restaurant_id)]);
    if (restaurantRows.length === 0) return res.status(400).json({ error: "Invalid restaurant" });
    let cuisineIdExists = false;
    try {
      const [columns] = await pool.query("SHOW COLUMNS FROM deals LIKE 'cuisine_id'");
      cuisineIdExists = columns.length > 0;
    ...
    if (cuisineIdExists && cuisine_id) {
      ...
      if (cuisineRows.length === 0) return res.status(400).json({ error: "Invalid cuisine" });
    ...
    if (cuisineIdExists) {
      insertQuery = `
        INSERT INTO deals (title, slug, description, restaurant_id, cuisine_id, image, discount_price, original_price,
          discount_percent, valid_from, valid_until, tags, has_customization, deal_type, is_featured, quantity_available, is_active)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`;
      insertValues = [
        title.trim(), slug, description || '', parseInt(restaurant_id), finalCuisineId, image, discountPriceNum,
        originalPriceNum, finalDiscountPercent, validFromMySQL, validUntilMySQL, tags || '', detectedHasCustomization,
        detectedDealType, is_featured === 'true' || is_featured === true, quantity_available || null,
        is_active === 'true' || is_active === true
      ];
    } else {
      insertQuery = `
        INSERT INTO deals (title, slug, description, restaurant_id, image, discount_price, original_price,
          discount_percent, valid_from, valid_until, tags, has_customization, deal_type, is_featured, quantity_available, is_active)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`;
      insertValues = [
        title.trim(), slug, description || '', parseInt(restaurant_id), image, discountPriceNum,
        originalPriceNum, finalDiscountPercent, validFromMySQL, validUntilMySQL, tags || '', detectedHasCustomization,
        detectedDealType, is_featured === 'true' || is_featured === true, quantity_available || null,
        is_active === 'true' || is_active === true
      ];
    ...
    const [newDeal] = await pool.query(
      `SELECT d.*, r.name as restaurant_name, r.image as restaurant_image FROM deals d LEFT JOIN restaurants r ON d.restaurant_id = r.id WHERE d.id = ?`,
      [result.insertId]
    );
...
app.patch("/api/deals/:id", upload.single('image'), async (req, res) => {
  // (Keep your existing PATCH deal logic – unchanged)
  try {
    const dealId = parseInt(req.params.id);
    if (isNaN(dealId) || dealId <= 0) return res.status(400).json({ error: "Invalid deal ID" });
    const [existingDeal] = await pool.query("SELECT id FROM deals WHERE id = ?", [dealId]);
    if (existingDeal.length === 0) return res.status(404).json({ error: "Deal not found" });
    const { title, description, restaurant_id, cuisine_id, discount_price, original_price, discount_percent,
      valid_from, valid_until, tags, has_customization, deal_type, is_active, is_featured, quantity_available } = req.body;
    const updates = [];
    const values = [];
    if (title !== undefined) { updates.push("title = ?"); values.push(title.trim()); const slug = nameToSlug(title); updates.push("slug = ?"); values.push(slug); }
    ...
    if (cuisineIdExists && cuisine_id !== undefined) { updates.push("cuisine_id = ?"); values.push(cuisine_id ? parseInt(cuisine_id) : null); }
    if (req.file) { const image = `/uploads/${req.file.filename}`; updates.push("image = ?"); values.push(image); }
...
app.post("/api/orders", async (req, res) => {
  ...
  try {
    ...
    if (restaurantCheck.length === 0) return res.status(400).json({ success: false, error: "Invalid or inactive restaurant" });
    ...
    for (const item of items) {
      ...
      if (item.deal_id) {
        await connection.query(`INSERT INTO order_items (order_id, deal_id, quantity, unit_price) VALUES (?, ?, ?, ?)`, [orderId, item.deal_id, quantity, unitPrice]);
      } else {
        const menuItemId = item.menu_item_id || item.id;
        if (!menuItemId) throw new Error("Item has neither menu_item_id nor deal_id");
        await connection.query(`INSERT INTO order_items (order_id, menu_item_id, quantity, unit_price) VALUES (?, ?, ?, ?)`, [orderId, menuItemId, quantity, unitPrice]);
      }
    }
    await connection.commit();
    connection.release();
    const [newOrder] = await pool.query(
      `SELECT o.*, r.name as restaurant_name, r.address as restaurant_address FROM orders o LEFT JOIN restaurants r ON o.restaurant_id = r.id WHERE o.id = ?`,
      [orderId]
    );
    const [orderItems] = await pool.query(
      `SELECT oi.*, mi.name as item_name, d.title as deal_name, d.description as deal_description
       FROM order_items oi LEFT JOIN menu_items mi ON oi.menu_item_id = mi.id LEFT JOIN deals d ON oi.deal_id = d.id WHERE oi.order_id = ?`,
      [orderId]
    );
